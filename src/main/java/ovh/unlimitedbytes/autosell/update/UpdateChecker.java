 ...
 * blocked; the result is marshalled back with {@link Minecraft#execute}),
...
public final class UpdateChecker {
	...
	/** Runs on the client thread via {@link Minecraft#execute}. */
	private static void announceIfOutdated(Minecraft client, String body, String localVersion) {
		try {
			...
			Component link = Component.translatable("bytesautosell.msg.update_link")
					.withStyle(style -> style
							.withColor(ChatFormatting.AQUA)
							.withUnderlined(true)
							.withClickEvent(new ClickEvent.OpenUrl(URI.create(releaseUrl)))
							.withHoverEvent(new HoverEvent.ShowText(Component.literal(releaseUrl))));
			client.player.sendSystemMessage(Component.translatable("bytesautosell.msg.update_available", remoteVersion)
